...
## Pre-Build: Claude Code Skills (1-2 дня)

Перед началом реализации — создаём и устанавливаем 8 скиллов. Скиллы ускоряют BUILD-фазу (написание кода агентов и промптов) и помогают в OPERATE-фазе (отладка и тестирование).

### Скиллы для создания (7 штук)

| # | Скилл | Тип | Статус | Назначение |
|---|-------|-----|--------|------------|
| 1 | `agent-builder` | BUILD | Создать | Шаблон создания агентов: BaseAgent → execute() → PipelineState. Генерирует boilerplate для нового агента за один промпт: class, execute, load_prompt, tests |
| 2 | `prompt-crafter` | BUILD | Создать | 38 принципов дизайна промптов из CLAUDE.md (секция "Prompt Design Principles"). При написании каждого промпта агента — skill подставляет релевантные принципы и hard bans |
| 3 | `pattern-auditor` | OPERATE | Создать | Интерактивная проверка текста на P1-P53 + F1-F8 + AC-1–AC-15. Выводит: нарушения, рекомендации, score. Используется для ручной отладки output'а pipeline |
| 4 | `ac-gate` | REFERENCE | Создать | 15 AC-правил в формате quick lookup. При работе с любым агентом — мгновенная справка по language/register/domain гейтам без поиска в CLAUDE.md |
| 5 | `academic-writer` | BUILD+RUNTIME | Создать | Повышение качества СОДЕРЖАНИЯ текстов (не анти-детекция, а ценность): глубина аргументации по доменам, критический анализ источников, предметная плотность (мин. фактов/цифр/имён на 500 слов), логические мосты между главами, domain-specific шаблоны (IT: архитектура системы, экономика: финанализ, право: анализ нормы). Megaprompt задаёт ФОРМУ — этот скилл задаёт СОДЕРЖАНИЕ |
| 6 | `academic-visualizer` | BUILD+RUNTIME | Создать | Качество визуализаций (БЛОК 16+): шаблоны таблиц по доменам (сравнительные, хронологические, аналитические), описания для figure placeholders (UML, ER, блок-схемы, графики), генерация правдоподобных данных (не "100, 200, 300" а реалистичные метрики с дисперсией), готовые SWOT/PEST/Porter's Five Forces, правила размещения визуализаций в тексте |
| 7 | `pattern-auditor-ru` | OPERATE | Создать (опционально) | Русскоязычная версия pattern-auditor: p7_russian banlist, p29_russian connectors, русские announcement openers, ГОСТ-специфичные проверки. Может быть объединён с pattern-auditor (#3) как language mode |

### Скилл для скачивания (1 штука)

| # | Скилл | Источник | Назначение |
|---|-------|----------|------------|
| 8 | `humanizer` | `git clone https://github.com/blader/humanizer` | Ручное тестирование humanization: 24 паттерна, two-pass audit. Используется для сравнения: pipeline output vs. skill output. НЕ production dependency — отладочный инструмент |

### Скилл для тестирования (1 штука, решение после теста)

| # | Скилл | Источник | Статус | Назначение |
|---|-------|----------|--------|------------|
| 9 | `cost-aware-llm-pipeline` | `everything-claude-code` repo | **ТЕСТ → РЕШЕНИЕ** | Оптимизация стоимости API-вызовов. Тестируем на нашем контексте по 3 критериям: (1) релевантность — даёт ли советы сверх Appendix B, (2) конфликты — не противоречит ли архитектуре 82 агентов, (3) ROI — экономит ли время при разработке API Gateway. Если 2/3 = да → оставляем. Иначе → удаляем |

### Связь скиллов с фазами реализации

| Скилл | Где используется | Фазы |
|-------|-----------------|------|
| `agent-builder` | Создание каждого агента (82 компонента) | Phase 0–6 |
| `prompt-crafter` | Написание промптов для всех агентов | Phase 1–5 |
| `pattern-auditor` | Тестирование output'а после каждой фазы | Phase 4–6 |
| `ac-gate` | Справка при реализации Language Gate и HM workers | Phase 4–6 |
| `academic-writer` | Улучшение промптов Content Workers + Phase 0B megaprompt | Phase 2–3 |
| `academic-visualizer` | Улучшение БЛОК 16 + Content Workers для визуализаций | Phase 2–3 |
| `humanizer` | Ручная отладка, сравнение с pipeline output | Phase 4–5 |
| `cost-aware-llm-pipeline` | API Gateway, CostReport (если прошёл тест) | Phase 6 |

### Порядок создания

1. **`agent-builder`** — нужен с Phase 0 (первый агент)
2. **`prompt-crafter`** — нужен с Phase 1 (первый промпт агента)
3. **`humanizer`** — скачать (`git clone`) — 1 минута
4. **`academic-writer`** — нужен к Phase 2 (Content Workers)
5. **`academic-visualizer`** — нужен к Phase 2 (визуализации в Content Workers)
6. **`ac-gate`** — нужен к Phase 4 (Language Gate)
7. **`pattern-auditor`** — нужен к Phase 4 (тестирование HM output)
8. **`cost-aware-llm-pipeline`** — тест к Phase 6 (API Gateway)
...
**Single-generation streams (text, essay, composition):**
- `mm_text.py`, `mm_essay.py`, `mm_composition.py` — каждый имеет SECTION_ORDER = `["full"]`
- Используют **один** worker (`text_*.py`, `essay_full.py`, `comp_full.py`), который вызывает `generator.generate(params)` (НЕ `generate_section()`) — полная генерация за один API-вызов
- ContentManager маршрутизирует их через тот же интерфейс `mm.execute(state)` — единообразно с multi-section streams
- `mm_text.py` дополнительно выбирает подтип worker'а (analytical, journalistic, review, descriptive) на основе `state.text_subtype`
...
|-------|------|--------|-----------------|--------|-----------------|
| Pre-Build | 1-2 дня | 8 скиллов | agent-builder, prompt-crafter, academic-writer, academic-visualizer, pattern-auditor, ac-gate, humanizer, cost-aware-llm-pipeline (тест) | — | 0 |
...
| **ИТОГО** | **~7-9 недель** | **~103 + 8 скиллов** | **82 компонента + 8 skills** | **~101 новых** | **0** |
